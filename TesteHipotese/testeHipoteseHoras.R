...
media_amostral <- mean(dados$hrsinternet)
desvio_padrao <- sd(dados$hrsinternet)
n <- nrow(dados)  # Tamanho da amostra

# Definindo a média hipotética e o nível de significância
media_hipotetica <- 3.5        # Média hipotética sob H0
...
x <- seq(-7, 7, length = 100)  # Gera uma sequência de -4 a 4
